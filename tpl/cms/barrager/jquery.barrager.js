...
function tanmu(url) {
     ...
     $.getJSON(url,function(data){
      ...
      function  barrager(){
		...
		$('.player_zanpian').barrager(items[index]);
	...
	$("#tamuform").qiresub({
		...
		onsucc : function(result) {
			...
			if (parseInt(result['code']) > 0) {
				tanmuone(Root + "index.php?s=home-barrage-index-t-1-id-"+ $("#barrage").data('id') + '') ;
			}else{
				$('img.validate-img').attr("src",verify());
				$("#validatedanmu .validate").trigger("click")
			}
		}
	}).post({
		url : Root + 'index.php?s=/home/barrage/add'
	});
...
if ($("#barrage").data('id') != undefined && $("#barrage").data('id') != null && $("#barrage").data('id') != '') {
			tanmu(Root+ "index.php?s=/home/barrage/index/id/" + $("#barrage").data('id'));
		} else {
			return false;
}
    $("#slider").toggle(
            function () {
                $(this).css("left","0");
				$(".m").toggleClass("on");
				$(".tanmu_m,.tamnu_open").toggleClass("one");
            ...
            function () {
				...
				$(".m").toggleClass("on");
				$(".tanmu_m,.tamnu_open").toggleClass("one");
				tanmu(Root +"index.php?s=home-barrage-index-id-" + Id + '');
            }
        );
});	